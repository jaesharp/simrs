		...
		try {
			...
			AMSession deploy = ams.openSession(isdAID)   // select SD & open secure channel
					...
					.install(sAID_CAP,                   // install application
							sAID_AppletClass,
							sAID_AppletInstance,
							new byte[] {0x05, 0x01, 0x02, 0x03, 0x04, 0x05}) // PIN
					.close();
			...
			TestScript testScript = new TestScript()
					...
					//Generate KeyPair
					.append(new CommandAPDU(0x80, 0x70, 0x00, 0x00, new byte[] {}, 256))
					...
					.append(new CommandAPDU(0x80, 0x30, 0x00, 0x00, new byte[] {0x64}, 256))
							//new ResponseAPDU(new byte[amount || counter || signature] { (byte)0x90, (byte)0x00}))
					...
					.append(new CommandAPDU(0x80, 0x40, 0x00, 0x00, new byte[] {0x32},256))
							//new ResponseAPDU(new byte[amount || counter || signature] { (byte)0x90, (byte)0x00}))
