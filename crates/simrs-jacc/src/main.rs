...
fn main() {
    ...
    if args[1] == "--version" || args[1] == "-V" {
        println!("simrs-jacc 0.1.0 (simrs JVA compiler)");
        return;
    }
...
fn run_disasm(path: &str) {
    ...
    match simrs_jacc::decompile::disassemble(&data) {
        Ok(asm) => print!("{asm}"),
        Err(e) => {
            eprintln!("error: {e}");
            std::process::exit(1);
...
fn run_decompile(path: &str) {
    ...
    match simrs_jacc::decompile::decompile(&data) {
...
fn compile_source(path: &str) -> Result<Vec<u8>, String> {
    ...
    let class = simrs_jacc::java_parser::parse_source(&source)?;
    ...
    Ok(simrs_jacc::cap::write_cap(&compiled))
}
...
fn compile_classfile(path: &str) -> Result<Vec<u8>, String> {
    ...
    let class = simrs_jacc::classfile::read_and_convert(&data, &default_aid)?;
    let compiled = simrs_jccompile::compile_class(&class).map_err(|errors| {
        errors
            .iter()
            .map(ToString::to_string)
...
fn print_usage() {
    eprintln!("Usage: simrs-jacc <input> [-o <output>] [--source-map]");
    eprintln!("       simrs-jacc --disasm <input.cap>");
    eprintln!("       simrs-jacc --decompile <input.cap>");
    eprintln!();
