...
fn disassemble_simple_return() {
    ...
    let asm = simrs_jacc::decompile::disassemble(&cap).unwrap();
...
fn decompile_simple_return() {
    ...
    let source = simrs_jacc::decompile::decompile(&cap).unwrap();
...
fn roundtrip_compile_decompile() {
    ...
    ";
    let class = simrs_jacc::java_parser::parse_source(source).unwrap();
    let compiled = simrs_jccompile::compile_class(&class).unwrap();
    let cap = simrs_jacc::cap::write_cap(&compiled);

    let decompiled = simrs_jacc::decompile::decompile(&cap).unwrap();
    assert!(
...
fn roundtrip_full_pipeline() {
    ...
    let asm = simrs_jacc::decompile::disassemble(&cap).unwrap();
    let src = simrs_jacc::decompile::decompile(&cap).unwrap();
...
proptest! {
    ...
    fn pbt_disassemble_never_panics(a in -128i8..127, b in -128i8..127, op_idx in 0usize..5) {
        ...
        let result = simrs_jacc::decompile::disassemble(&cap);
    ...
    fn pbt_decompile_never_panics(a in -128i8..127, b in -128i8..127, op_idx in 0usize..5) {
        ...
        let cap = cap_from_bytecodes(&arith_bytecodes(a, b, op));
        let result = simrs_jacc::decompile::decompile(&cap);
