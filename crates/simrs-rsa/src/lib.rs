...
fn pkcs1_encrypt_pad(msg: &[u8], em: &mut [u8], rand_fill: &mut dyn FnMut(&mut [u8])) -> usize {
    let em_len = em.len();
...
mod tests {
    ...
    fn raw_encrypt_decrypt_roundtrip() {
        ...
        assert_eq!(
            recovered.limbs, plaintext.limbs,
            "raw RSA round-trip failed"
        );
    }
    ...
    fn encrypt_decrypt_roundtrip() {
        ...
        let ct_len = encrypt_pkcs1(&pub_key, plaintext, &mut ciphertext, &mut test_rand_fill);
        assert_eq!(ct_len, 64);
