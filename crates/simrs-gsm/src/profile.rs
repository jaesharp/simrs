...
/// Encoding: BCD-nibble-swapped per GSM 11.11 clause 10.1.1.
...
pub static EF_SST: EfDef = EfDef::transparent(
    ...
    None,
    &[
        0xFF, 0x3F, 0xFF, 0xFF, 0x3F, 0x00, 0x3F, 0x0F,
        0x30, 0x0C, 0x00, 0x00, 0x00, 0xC0,
    ],
);
...
pub static EF_SPN: EfDef = EfDef::transparent(
    ...
    None,
    &[
        0x00, 0x73, 0x77, 0x53, 0x49, 0x4D, 0x20, 0x62, 0x79,
        0x20, 0x54, 0x6F, 0x6D, 0x61, 0x73, 0x7A, 0xFF,
    ],
);
...
mod tests {
    ...
    fn gsm_mf_has_3_children() {
        assert_eq!(
            REFERENCE_MF_GSM.children.len(), 3,
    ...
    fn gsm_df_telecom_is_empty() {
        ...
        assert!(DF_TELECOM.children.is_empty(),
    ...
    fn gsm_fsdata_init_succeeds() {
        ...
        assert!(result.is_ok(),
            "FsData::init failed for GSM profile: {:?}", result.err());
    }
    ...
    fn df_gsm_ef_at_correct_fid() {
        ...
        for (fid, exp_len) in &expected {
            let found = DF_GSM.children.iter().find_map(|c| {
                if let FileRef::Ef(ef) = c {
                    if ef.fid().value() == *fid { Some(*ef) } else { None }
                } else {
                    None
                }
            });
            assert!(found.is_some(), "EF {fid:#06X} not found in DF.GSM");
            assert_eq!(found.unwrap().data().len(), *exp_len,
    ...
    fn df_gsm_standard_efs_at_correct_fid() {
        ...
        for (fid, exp_len) in &expected {
            ...
            assert!(found.is_some(), "EF {fid:#06X} not found in DF.GSM (standard)");
            assert_eq!(found.unwrap().data().len(), *exp_len,
                "EF {fid:#06X} data length mismatch");
        }
    }
    ...
    fn ef_phase_is_phase2_plus() {
        assert_eq!(EF_PHASE.data()[0], 0x03,
    ...
    fn ef_loci_update_status() {
        assert_eq!(EF_LOCI.data()[10], 0x03,
