...
| JCVM bytecode | JCVM 3.2 | Most opcodes implemented; 16 wide-offset conditional branches added 2026-05-03 | **KNOWN COMPLIANCE GAP** -- ~20 opcodes use non-spec values, ~20 spec opcodes missing, internal `astore_n` vs `sstore_0..2` collision, and the wide-branch range (codebase 0x96..0xA5) is unverified against spec (recollection: 0x98..0xA7). Catalogued in [07-jcvm-opcode-compliance.md](07-jcvm-opcode-compliance.md). Internally consistent; **not interoperable with Oracle-converter-emitted CAP files** until Phase A migration lands. |
| JCVM CAP file format | JC 3.2 (CAP v2.3+) | Component-tagged parser MVP (10 of 13 components surfaced) + simplified blob, both via dispatcher | `simrs-jcvm::cap::parse_cap` auto-detects format. The component-tagged path (`cap::components`) now parses Header, Method, Descriptor, ConstantPool, Applet, Import, Export, RefLocation, StaticField (summary), and Class (MVP); only Directory, Debug, and StaticResources are still walk-and-skip. ConstantPool entries store raw 4-byte form and decode on demand via `CpInfo::as_*` helpers. Component tags 1..13 and CP tags 1..6 verified spec-aligned. 21 proptest blocks across the 7 new component parsers cover round-trip, variable-walk, truncation-fuzz, and snapshot save/restore. **Remaining**: token resolution at invoke time, full Class interface_info decoding, full StaticField image. **Possible structural deviations** flagged in [07-jcvm-opcode-compliance.md](07-jcvm-opcode-compliance.md): TYPE_DESC_VOID may be wrong nibble + wrong encoding shape; FieldInfo::type_token may be wrong width; ACC_SHAREABLE/ACC_REMOTE doc claim may be wrong (runtime unaffected). |
...
| Toolchain (jacc/jccompile/jcasm) | JC 3.2 converter behaviour | s/i opcode emission complete; CAP writer emits all 13 components, strictly spec-compliant for standalone applets | Annotations tokenised but discarded. The writer was made strictly conformant in this session: Export (tag 10), Debug (tag 12), StaticResources (tag 13) added; RefLocation, StaticField, Directory `static_field_size_info`, and Class `package_method_table_count` bodies brought up to spec. |
