...
pub fn query_pin1_retries(world: &mut SimWorld) -> u8 {
    let cmd = simrs_adversarial_countervalidation::apdu::verify_query(PinKey::PIN1).build();
...
pub fn query_puk1_retries(world: &mut SimWorld) -> u8 {
    let cmd = simrs_adversarial_countervalidation::apdu::unblock_query(PinKey::PIN1).build();
    let (sw1, sw2) = send_apdu_sw(world.sim_mut(), &cmd);
    if sw1 == 0x63 {
        sw2 & 0x0F
    } else if sw1 == 0x69 && sw2 == 0x83 {
...
pub fn send_terminal_profile(world: &mut SimWorld) {
    let cmd = simrs_adversarial_countervalidation::apdu::terminal_profile(&[0xFF; 4]).build();
    do_send_apdu(world, &cmd);
}
